...
const TClass = ({ resources, classesData }) => {
  ...
  const handleCreateDialogChange = async (flag, value) => {
    try {
      if (flag) {
        ...
        enqueueSnackbar('Successfully class created!', {
          variant: 'success'
        });
      }
    ...
    <Box className={classes.root}>
      ...
      <LoadingCard loading={loadingPage} height={`calc(100vh - 200px)`}>
        <main className={classes.main} ref={mainRef}>
          ...
          <CreateClass open={openCreate} onChange={handleCreateDialogChange} />
          <PreviewClass
            open={openPreview}
